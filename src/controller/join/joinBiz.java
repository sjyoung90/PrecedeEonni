...
/**
 * Servlet implementation class joinBiz
 */
public class joinBiz extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String	kind = up.getParameter("kind");
		...
		*/
		...
		insBiz in = new insBiz();
			
		//ServletContext sc = getServletContext();
		conn c = new conn();
		try {
			//Connection joinConn= (Connection) sc.getAttribute("connDB");
			...
			if(result.next())	{
				...
				if(in.insertBizToDB(id, pw, name, role, college, major, fileName, phone, intro, way, 
						offLine, company, kind, position, joinConn) == 3){
					System.out.println("business : join sucess");
					//resultToJSP = 2;
