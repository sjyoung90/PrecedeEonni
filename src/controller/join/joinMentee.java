...
/**
 * Servlet implementation class joinMentee
 * 동생으로 회원가입 서블릿
 */
public class joinMentee extends HttpServlet {
		...
		*/
		...
		try {
			...
			if(result.next())	{
				...
				if(in.insertMemberToDB(id, pw, name, role, college, major, fileName, phone, intro, wayList, joinConn) == 2){
					System.out.print("join sucess");
					//resultToJSP = 2;
					HttpSession session = request.getSession();
					session.setAttribute("currentID", Integer.toString(id));	//회원가입후 바로 로그인상태로 만들기
					session.setAttribute("role", "동생");
					...
					if(rsBase == null)	System.out.println("rsBase null");
						...
						if(role.equals("동생")){
							...
							session.setAttribute("way", rsBase.getString("way"));
						}*/
					}
				}
			}
